  ...
  return (
    <div className="pt-4 space-y-3">
      {Object.entries(techStackGroups).map(([category, technologies]) => (
        <div key={category}>
          <h5 className={`${secondaryTextColor} text-xs sm:text-sm font-medium mb-2 uppercase tracking-wider`}>{category}</h5>
...
const TechStack = ({ registerSection }: TechStackProps) => {
  ...
  if (inRetro) {
    return (
      <SectionPanel id="stack" registerSection={registerSection} className="retro-panel">
        <div className="retro-panel__titlebar">
          <span className="retro-panel__title" id="heading-stack">
            Installed components
          </span>
        </div>
        <div className="retro-panel__body">
          {Object.entries(techStackGroups).map(([category, technologies]) => (
            <div key={category} className="retro-stack-group">
              <h5 className="retro-stack-group__label">{category}</h5>
